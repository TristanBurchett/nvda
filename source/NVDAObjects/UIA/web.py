...
class UIAWebTextInfo(UIATextInfo):
	...
	def removeClickableStateOnDescendants(self, fields, textInfos, controlTypes):
		clickableField = None
		for field in fields:
	...
	def handleIncorrectWhitespace(self, fields):
		numFields = len(fields)
		index = 0
	...
	def getTextWithFields(  # noqa: C901
		...
		self.removeClickableStateOnDescendants(fields, textInfos, controlTypes)
		
		# Chop extra whitespace off the end incorrectly put there by Edge
		self.handleIncorrectWhitespace(fields)
		...
		self.removeContentFromFieldsWithContentAttr(fields, textInfos)
		return fields
